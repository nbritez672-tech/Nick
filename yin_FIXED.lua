...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    function Window:CreateTab(nameSpanish, nameEnglishOrIcon, iconAsset)
        ...
        function Tab:CreateFloatingToggle(textSpanish, textEnglishOrDefault, defaultOrCallback, callback)
            ...
            --// Permite vincular un Slider a este Toggle.
            --// Cuando el Toggle se añade a Favoritos, el Slider vinculado
            --// también aparece en Favoritos justo debajo.
            ...
            tog.LinkSlider = function(self, sliderObj)
                ...
                --// Si ya es favorito cuando se llama LinkSlider, sincronizar inmediatamente
                if isFavorite then
                    syncFavoriteRow()
                end
            end

            --// Extender syncFavoriteRow para incluir slider vinculado
            local _originalSyncFavoriteRow = syncFavoriteRow
            syncFavoriteRow = function()
                _originalSyncFavoriteRow()
                --// Manejar slider vinculado
                if _linkedSliderFavRow then
                    pcall(function() _linkedSliderFavRow:Destroy() end)
                    _linkedSliderFavRow = nil
                end
                if isFavorite and _linkedSliderRef and AutoTabFavoritosRef then
                    --// Crear slider espejo en Favoritos
    ...
    --// ════════════════════════════════════════════════════════════════
    --// GRID DE TEMAS CON VISTA PREVIA (v28 PRO)
    --// Tarjetas en cuadrícula: cada una muestra una previsualización real
    --// de los colores del tema (fondo / secundario / acento) y se marca
    --// con un check cuando es el tema activo.
    --// ════════════════════════════════════════════════════════════════
    ...
    --// Guardamos referencia {card, nombreTema, ...} para filtrar y resaltar el activo
    local ThemeButtons = {}
    ...
    for i, tema in ipairs(temas) do
        ...
        --// Franja de vista previa: fondo + secundario + acento del tema real
        ...
        --// Insignia "✓" que solo se ve en el tema activo
    ...
    --// Filtro dinámico: oculta las tarjetas que no coincidan con la búsqueda
