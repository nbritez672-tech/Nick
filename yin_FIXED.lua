...
do
    ...
    local function SendMessage()
    	...
    	--// COMANDO /player mensaje — solo para Mousoza
    	if localPlayer.Name == "Mousoza" and messageText:match("^/(%S+)%s+(.+)$") then
    		local targetName, realMessage = messageText:match("^/(%S+)%s+(.+)$")
    		task.spawn(function()
    			-- Buscar el userId del target via API de Roblox
    			local ok, result = pcall(function()
    				return UniversalRequest({
    					Url    = "https://users.roblox.com/v1/usernames/users",
    					Method = "POST",
    					Headers = { ["Content-Type"] = "application/json" },
    					Body   = HttpService:JSONEncode({ usernames = { targetName }, excludeBannedUsers = false }),
    				})
    			end)
    			...
    			if not decodeOk or not data or not data.data or not data.data[1] then return end
    			local targetId   = data.data[1].id
    			local targetDisplayName = data.data[1].name
    			-- Solo mandar al backend — el polling lo mostrará para todos incluyendo Mousoza
    			BackendSendMessage(targetDisplayName, targetId, realMessage)
    			MessageInput.Text = ""
