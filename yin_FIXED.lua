...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    local function disconnectOpenYinYangConnections()
        for _, connection in ipairs(OpenYinYang.Connections or {}) do
            pcall(function() connection:Disconnect() end)
        end
        OpenYinYang.Connections = {}
    end
    ...
    local function clearOpenYinYangControl()
        ...
        OpenYinYang.DragZone = nil
        OpenYinYang.OpenZone = nil
    end
    ...
    local function ApplyLogoIcon(iconName)
        ...
        --// 2. Desconectar y destruir el control especial anterior y sus capas.
        local wasOpenYinYangMode = OpenYinYang.Mode == true
        ...
        OpenYinYang.Mode = iconData.Control == "OpenYinYang"
        OpenYinYang.Connections = {}
        ...
        if OpenYinYang.Mode then
            ...
            OpenYinYang.OpenZone = mk("TextButton", {
            ...
            layerParent = OpenYinYang.DragZone
            layerSize = UDim2.new(0, iconSize, 0, iconSize)
            layerPosition = UDim2.new(0.5, -iconSize / 2, 0.5, -iconSize / 2)
            layerZOffset = 34

            table.insert(OpenYinYang.Connections, OpenYinYang.OpenZone.Activated:Connect(function()
                if OpenYinYang.RequestToggleLibrary then
                    OpenYinYang.RequestToggleLibrary()
                end
            end))

            local drag = false
            local dragStart, startPos
            table.insert(OpenYinYang.Connections, OpenYinYang.DragZone.InputBegan:Connect(function(input)
            ...
            table.insert(OpenYinYang.Connections, UserInputService.InputChanged:Connect(function(input)
                if drag and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                    local delta = input.Position - dragStart
                    ToggleButton.Position = UDim2.new(0, startPos.X.Offset + delta.X, 0, startPos.Y.Offset + delta.Y)
                end
            end))
            table.insert(OpenYinYang.Connections, UserInputService.InputEnded:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    drag = false
                end
            end))
        ...
        for i, layerDef in ipairs(iconData.Layers) do
            ...
            local label = mk("ImageLabel", {
                ...
                ZIndex               = (OpenYinYang.Mode and layerZOffset or 30) + i,
            })
    ...
    OpenYinYang.RequestToggleLibrary = function()
        ...
        if OpenYinYang.Mode and uiVisible then
            return
        end
