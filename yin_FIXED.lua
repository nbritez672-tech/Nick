...
local IconCatalog = {
    Version = 1,
    Order = { "ClassicYinYang", "OpenYinYang" },
    Icons = {
        ClassicYinYang = {
...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    local openYinYangMode = false
    local openYinYangDragZone = nil
    local openYinYangOpenZone = nil
    local openYinYangConnections = {}
    local requestToggleLibrary = nil

    local function disconnectOpenYinYangConnections()
        for _, connection in ipairs(openYinYangConnections) do
            pcall(function() connection:Disconnect() end)
        end
        table.clear(openYinYangConnections)
    end

    local function clearOpenYinYangControl()
        disconnectOpenYinYangConnections()
        ...
        openYinYangDragZone = nil
        openYinYangOpenZone = nil
    end
    ...
    local function ApplyLogoIcon(iconName)
        ...
        local wasOpenYinYangMode = openYinYangMode
        clearOpenYinYangControl()
        openYinYangMode = iconData.Control == "OpenYinYang"

        local layerParent = ToggleButton
        local layerSize
        local layerPosition
        local layerZOffset = 30

        if openYinYangMode then
            local pillWidth = iconData.Width or 246
            local handleWidth = iconData.HandleWidth or 58
            local iconSize = iconData.IconSize or 24

            ToggleButton.Size = UDim2.new(0, pillWidth, 0, iconData.Height or 52)
            if not wasOpenYinYangMode then
                ToggleButton.Position = UDim2.new(0.5, -(pillWidth / 2), 0, 18)
            end
            ToggleButton.BackgroundColor3 = iconData.BackgroundColor or Color3.fromRGB(7, 9, 13)
            local openYinYangStroke = ToggleButton:FindFirstChildOfClass("UIStroke")
            if openYinYangStroke then
                openYinYangStroke.Color = Color3.fromRGB(226, 231, 242)
                openYinYangStroke.Thickness = 2
                openYinYangStroke.Transparency = 0.08
            end
            ToggleButton.AutoButtonColor = false
            ToggleButton.ClipsDescendants = true

            openYinYangDragZone = mk("Frame", {
                Name = "OpenYinYangDragZone",
                Parent = ToggleButton,
            ...
            openYinYangOpenZone = mk("TextButton", {
                Name = "OpenYinYangOpenZone",
                Parent = ToggleButton,
                Size = UDim2.new(1, -handleWidth, 1, 0),
                Position = UDim2.new(0, handleWidth, 0, 0),
                BackgroundTransparency = 1,
                Text = iconData.OpenText or "Open YinYang",
            ...
            layerParent = openYinYangDragZone
            layerSize = UDim2.new(0, iconSize, 0, iconSize)
            layerPosition = UDim2.new(0.5, -iconSize / 2, 0.5, -iconSize / 2)
            layerZOffset = 34

            table.insert(openYinYangConnections, openYinYangOpenZone.Activated:Connect(function()
                if requestToggleLibrary then
                    requestToggleLibrary()
                end
            end))

            local drag = false
            local dragStart, startPos
            table.insert(openYinYangConnections, openYinYangDragZone.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    if not IconoCongelado then
                        drag = true
            ...
            table.insert(openYinYangConnections, UserInputService.InputChanged:Connect(function(input)
                if drag and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                    local delta = input.Position - dragStart
                    ToggleButton.Position = UDim2.new(0, startPos.X.Offset + delta.X, 0, startPos.Y.Offset + delta.Y)
                end
            end))
            table.insert(openYinYangConnections, UserInputService.InputEnded:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    drag = false
                end
            end))
            ...
            if wasOpenYinYangMode then
                ToggleButton.Position = UDim2.new(0, 24, 0, 70)
            end
            ToggleButton.BackgroundColor3 = Theme.Accent
            local normalIconStroke = ToggleButton:FindFirstChildOfClass("UIStroke")
            if normalIconStroke then
                normalIconStroke.Color = Theme.Accent
                normalIconStroke.Thickness = 1.5
                normalIconStroke.Transparency = 0
            end
        ...
        for i, layerDef in ipairs(iconData.Layers) do
            ...
            local label = mk("ImageLabel", {
                ...
                ZIndex               = (openYinYangMode and layerZOffset or 30) + i,
            })
    ...
    requestToggleLibrary = function()
        --// El icono especial usa únicamente el texto como zona de apertura.
        if toggleEyesOnClick then
            toggleEyesOnClick()
        end

        --// En la pill, "Open YinYang" abre; el cierre sigue estando en la UI.
        if openYinYangMode and uiVisible then
            return
        end
