        ...
        if iconData.Type == "Capsule" then
            ...
            do
                ...
                if _capStroke then
                    ...
                    --// Mismo mecanismo que buildGlowOnStroke (usado en el resto de la librería):
                    --// un UIGradient con su Offset animado en loop infinito. El borde es SÓLIDO
                    --// BLANCO por defecto; lo que se mueve es una franja NEGRA ancha (transparencia
                    --// alta) que recorre el borde. (Antes estaba invertido: mayormente transparente
                    --// con una franja blanca angosta — corregido a pedido.)
                    local _capGrad = Instance.new("UIGradient")
                    _capGrad.Color = ColorSequence.new(Color3.fromRGB(255, 255, 255))
                    _capGrad.Transparency = NumberSequence.new({
                        NumberSequenceKeypoint.new(0.00, 0.05),  -- sólido
                        NumberSequenceKeypoint.new(0.20, 0.05),  -- sólido
                        NumberSequenceKeypoint.new(0.40, 0.90),  -- entra la franja negra
                        NumberSequenceKeypoint.new(0.60, 0.90),  -- franja negra ancha
                        NumberSequenceKeypoint.new(0.80, 0.05),  -- vuelve a sólido
                        NumberSequenceKeypoint.new(1.00, 0.05),  -- sólido
                    })
                    _capGrad.Offset = Vector2.new(-1.5, 0)
                    _capGrad.Parent = _capStroke

                    local _capSweep = TweenService:Create(
                        _capGrad,
                    ...
                    _capsulePulse:Play()
                    ActiveGlowTweens[_key] = { _capsulePulse, _capSweep }
                end
            end
            return
        end
