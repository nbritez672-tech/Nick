...
local function LoadIcons()
    ...
    if dlOk and type(dlResult) == "string" and #dlResult > 20 then
        ...
        pcall(function()
            if writefile then
                writefile(ICONS_CACHE_FILE, rawData)
                print("[YinYang Icons] 💾 Caché actualizada")
            end
        end)
    else
        --// PASO 2: Descarga falló → intentar caché local
        print("[YinYang Icons] ⚠️ Descarga falló, intentando caché...")
        pcall(function()
            if readfile and isfile and isfile(ICONS_CACHE_FILE) then
                rawData = readfile(ICONS_CACHE_FILE)
                print("[YinYang Icons] 📁 Usando caché local")
            end
        end)
    end

    --// PASO 3: Si tenemos datos, procesarlos
    if rawData then
        local parseOk, data = pcall(function()
            return loadstring(rawData)()
        end)
...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    local CapsuleState = {Active = false, Connections = {}, Toggle = nil, OpenOnly = nil, DragZone = nil, OpenZone = nil}

    local function ApplyLogoIcon(iconName)
        --// 1. Desconectar motor, sonido, cápsula y borde animado anteriores
        ...
        for _, connection in ipairs(CapsuleState.Connections) do
            pcall(function() connection:Disconnect() end)
        end
        for index = #CapsuleState.Connections, 1, -1 do
            CapsuleState.Connections[index] = nil
        end
        for _, name in ipairs({"OldDragZone", "OldDivider", "OldOpenZone"}) do
            local child = ToggleButton:FindFirstChild(name)
            if child then child:Destroy() end
        end
        CapsuleState.DragZone = nil
        CapsuleState.OpenZone = nil
        CapsuleState.Toggle = nil
        stopGlowTweens(tostring(ToggleButton))
        cleanGlowLayers(ToggleButton)
        ...
        if not iconData or not iconData.Layers then return end

        local layerParent = ToggleButton
        local layerSize = nil
        local layerPosition = nil
        local layerZIndex = 30
        local wasCapsule = CapsuleState.Active
        CapsuleState.Active = iconData.Type == "Capsule"

        if CapsuleState.Active then
            local pillWidth = iconData.Width or 248
            local pillHeight = iconData.Height or 68
            local handleWidth = iconData.HandleWidth or 58
            local iconSize = iconData.IconSize or 28

            ToggleButton.Size = UDim2.fromOffset(pillWidth, pillHeight)
            if not wasCapsule then
                local viewportWidth = 0
                pcall(function()
                    local camera = workspace.CurrentCamera
                    if camera then viewportWidth = camera.ViewportSize.X end
                end)
                if viewportWidth <= 0 then viewportWidth = ScreenGui.AbsoluteSize.X end
                if viewportWidth <= 0 then viewportWidth = pillWidth end
                ToggleButton.Position = UDim2.fromOffset(math.floor((viewportWidth - pillWidth) / 2), 6)
            end
            ToggleButton.BackgroundColor3 = Color3.fromRGB(7, 9, 13)
            ToggleButton.BackgroundTransparency = 0
            ToggleButton.AutoButtonColor = false
            ToggleButton.ClipsDescendants = true
            local capsuleStroke = ToggleButton:FindFirstChildOfClass("UIStroke")
            if capsuleStroke then
                capsuleStroke.Thickness = 2.2
                capsuleStroke.Transparency = 0.05
                capsuleStroke.Color = Theme.Accent
            end

            CapsuleState.DragZone = mk("Frame", {
                Name = "OldDragZone",
                Size = UDim2.new(0, handleWidth, 1, 0),
            ...
            mk("Frame", {
                ...
                Position = UDim2.new(0, handleWidth, 0, 8),
                BackgroundColor3 = Color3.fromRGB(55, 58, 68),
                BackgroundTransparency = 0.12,
                BorderSizePixel = 0,
                ZIndex = 32,
            }, ToggleButton)

            CapsuleState.OpenZone = mk("TextButton", {
                Name = "OldOpenZone",
                Size = UDim2.new(1, -handleWidth, 1, 0),
                Position = UDim2.new(0, handleWidth, 0, 0),
                BackgroundTransparency = 1,
                Text = "Open YinYang",
            ...
            local openConnection = CapsuleState.OpenZone.Activated:Connect(function()
                if CapsuleState.OpenOnly then CapsuleState.OpenOnly() end
            end)
            table.insert(CapsuleState.Connections, openConnection)

            local drag = false
            local dragStart, startAbsolute
            table.insert(CapsuleState.Connections, CapsuleState.DragZone.InputBegan:Connect(function(input)
                if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and not IconoCongelado then
                    drag = true
                    dragStart = input.Position
                    startAbsolute = ToggleButton.AbsolutePosition
                end
            end))
            table.insert(CapsuleState.Connections, UserInputService.InputChanged:Connect(function(input)
                if drag and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                    local delta = input.Position - dragStart
                    ToggleButton.Position = UDim2.fromOffset(startAbsolute.X + delta.X, startAbsolute.Y + delta.Y)
                end
            end))
            table.insert(CapsuleState.Connections, UserInputService.InputEnded:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    drag = false
                end
            end))

            layerParent = CapsuleState.DragZone
            layerSize = UDim2.new(0, iconSize, 0, iconSize)
            layerPosition = UDim2.new(0.5, -iconSize / 2, 0.5, -iconSize / 2)
            layerZIndex = 34
            buildAnimatedBorder(ToggleButton, Theme.Accent, UDim.new(0, 999), true)
        else
            ToggleButton.Size = UDim2.new(0, 42, 0, 42)
            ToggleButton.BackgroundColor3 = Theme.Accent
            ToggleButton.AutoButtonColor = true
            ToggleButton.ClipsDescendants = false
            local normalStroke = ToggleButton:FindFirstChildOfClass("UIStroke")
            if normalStroke then
                local oldGradient = normalStroke:FindFirstChildOfClass("UIGradient")
                if oldGradient then oldGradient:Destroy() end
                normalStroke.Color = Theme.Accent
        ...
        for i, layerDef in ipairs(iconData.Layers) do
            ...
            local label = mk("ImageLabel", {
                Parent               = layerParent,
                Size                 = layerSize or UDim2.new(sc, 0, sc, 0),
                Position             = layerPosition or UDim2.new(off, 0, off, 0),
                BackgroundTransparency = 1,
                Image                = layerDef.Image or "",
                ZIndex               = layerZIndex + i,
            })
    ...
    CapsuleState.OpenOnly = function()
        if windowTweenBusy or uiVisible then
            return
        end
    ...
    ToggleButton.MouseButton1Click:Connect(function()
        if CapsuleState.Active then return end
        CapsuleState.Toggle()
    end)
    ...
    local function CreateIconCard(iconName, iconData, order)
        ...
        local lang      = LanguageSystem and LanguageSystem.CurrentLanguage or "es"
        local labelText = (lang == "en" and iconData.LabelEN) or iconData.LabelES or iconName
