        ...
        if iconData.Type == "Capsule" then
            ...
            do
                ...
                if _capStroke then
                    ...
                    --// Gradiente: sólido en las puntas curvas, se desvanece en el centro
                    --// de cada línea recta (arriba y abajo). Calculado sobre el perímetro
                    --// real del botón (185x42, radio 21): línea recta = 34.22% del perímetro,
                    --// punta curva = 15.78%. Si el desvanecido queda desplazado, ajustar estos
                    --// keypoints (mapean 0→1 recorriendo el perímetro completo en sentido horario).
                    local _capGrad = Instance.new("UIGradient")
                    _capGrad.Transparency = NumberSequence.new({
                        NumberSequenceKeypoint.new(0.0000, 0),
                        NumberSequenceKeypoint.new(0.1403, 0.05),
                        NumberSequenceKeypoint.new(0.1711, 0.85),  -- centro linea de arriba
                        NumberSequenceKeypoint.new(0.2019, 0.05),
                        NumberSequenceKeypoint.new(0.3422, 0),     -- fin linea / inicio punta
                        NumberSequenceKeypoint.new(0.5000, 0),     -- fin punta / inicio linea abajo
                        NumberSequenceKeypoint.new(0.6403, 0.05),
                        NumberSequenceKeypoint.new(0.6711, 0.85),  -- centro linea de abajo
                        NumberSequenceKeypoint.new(0.7019, 0.05),
                        NumberSequenceKeypoint.new(0.8422, 0),
