...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    local function ApplyLogoIcon(iconName)
        ...
        if iconData.Type == "Capsule" then
            ...
            --// Borde animado — reutiliza buildAnimatedBorder igual que la ventana principal
            --// Color fijo blanco (no Theme.Accent) y grosor 2px, solo para la cápsula
            local _capStroke = ToggleButton:FindFirstChildOfClass("UIStroke")
            if _capStroke then _capStroke.Thickness = 2 end
            buildAnimatedBorder(ToggleButton, Color3.fromRGB(200, 200, 220), UDim.new(1, 0))

            --// Fix del pulso: buildGlowOnStroke crea un pulseTween que va de Transparency 0 a 0.0
            --// (no anima nada, el stroke ya parte en 0). Se reemplaza SOLO para este ícono,
            --// sin tocar la función compartida (así no afecta a otros íconos ni a las ventanas flotantes).
            do
                local _key = tostring(ToggleButton)
                local _tweens = ActiveGlowTweens[_key]
                if _tweens and _tweens[2] and _capStroke then
                    _tweens[2]:Cancel()
