...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    function Window:CreateTab(nameSpanish, nameEnglishOrIcon, iconAsset)
        ...
        --// ════════════════════════════════════════════════════════════════════
        --// SCRIPT TOGGLE — Scripts externos con toggle ON/OFF
        ...
        function Tab:CreateScriptToggle(textSpanish, textEnglish, url, scriptKey)
            ...
            --// Mismo wrapper que CreateButton (ClipsDescendants para el ripple)
            local BtnWrapper = mk("Frame", {
                Size = UDim2.new(1, 0, 0, 36),
                BackgroundTransparency = 1,
                BorderSizePixel = 0,
            ...
            --// Indicador de estado activo (punto verde, esquina superior derecha)
            local ActiveDot = mk("Frame", {
            ...
            corner(ActiveDot, 999)

            --// Ripple — idéntico al de CreateButton
            local function spawnRipple(inputPosition)
                local btnPos  = Btn.AbsolutePosition
                local btnSize = Btn.AbsoluteSize
                local relX    = inputPosition.X - btnPos.X
                ...
                local ex = TweenService:Create(Ripple,
                    TweenInfo.new(0.55, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                    { Size = UDim2.fromOffset(maxR, maxR), BackgroundTransparency = 0.72 })
                ex:Play()
            ...
            --// Hover / Touch — igual que CreateButton
            track(Btn.MouseEnter:Connect(function()
            ...
            track(Btn.InputEnded:Connect(function(inp)
                if inp.UserInputType == Enum.UserInputType.Touch then
                    TweenService:Create(Btn, TweenInfo.new(0.20, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                        { BackgroundTransparency = 0 }):Play()
                end
            ...
            Btn.MouseButton1Click:Connect(function()
                ...
                if not active then
                    ...
                    active = true
                    TweenService:Create(Btn, TweenInfo.new(0.18), { BackgroundColor3 = Theme.ToggleOn }):Play()
                    TweenService:Create(ActiveDot, TweenInfo.new(0.18), { BackgroundTransparency = 0 }):Play()
                    task.spawn(function()
                        ...
                        if not ok then
                            ...
                            active = false
                            TweenService:Create(Btn, TweenInfo.new(0.18), { BackgroundColor3 = Theme.Secondary }):Play()
                            TweenService:Create(ActiveDot, TweenInfo.new(0.18), { BackgroundTransparency = 1 }):Play()
    ...
    --// Scripts externos — cada uno es un toggle ON/OFF independiente.
    --// Para agregar uno nuevo: copiar el bloque de abajo con la nueva url y scriptKey.
    --// El scriptKey debe coincidir con _G["_YY_STOP_<scriptKey>"] dentro del script.
    TabClassic:CreateScriptToggle(
        "Ocultar GUIs", "Hide GUIs",
        "https://raw.githubusercontent.com/Yinyangzx/yin-classic/refs/heads/main/YinYang_ClassicOptions.lua",
        "HideGuis"
    )
    end
