    ...
    local function ApplyLogoIcon(iconName)
        ...
        if iconData.Type == "Capsule" then
            ...
            do
                ...
                warn("[YinYang DEBUG] Entrando a bloque de borde de cápsula. _capStroke encontrado:", _capStroke ~= nil)
                if _capStroke then
                    local _oldGrad = _capStroke:FindFirstChildOfClass("UIGradient")
                    if _oldGrad then _oldGrad:Destroy() end
                    _capStroke.Thickness    = 3
                    _capStroke.Color        = Color3.fromRGB(200, 200, 220)
                    _capStroke.Transparency = 0
                    _capStroke.Enabled      = true
                    warn(string.format(
                        "[YinYang DEBUG] Stroke seteado -> Thickness=%s Color=%s Transparency=%s Enabled=%s Parent=%s",
                        tostring(_capStroke.Thickness),
                        tostring(_capStroke.Color),
                        tostring(_capStroke.Transparency),
                        tostring(_capStroke.Enabled),
                        tostring(_capStroke.Parent and _capStroke.Parent.Name)
                    ))
                    ...
                    task.delay(2, function()
                        if _capStroke and _capStroke.Parent then
                            warn(string.format(
                                "[YinYang DEBUG] 2s despues -> Thickness=%s Color=%s Transparency=%s Enabled=%s",
                                tostring(_capStroke.Thickness),
                                tostring(_capStroke.Color),
                                tostring(_capStroke.Transparency),
                                tostring(_capStroke.Enabled)
                            ...
                            warn("[YinYang DEBUG] 2s despues -> el stroke ya NO EXISTE (fue destruido o el Parent cambio)")
                        end
                    end)
                else
                    warn("[YinYang DEBUG] No se encontro ningun UIStroke como hijo directo de ToggleButton")
                end
            end
            return
        end
