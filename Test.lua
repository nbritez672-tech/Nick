...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    local function ApplyLogoIcon(iconName)
        ...
        if iconData.Type == "Capsule" then
            ...
            --// Borde de la cápsula: pulso propio, SIN reutilizar buildAnimatedBorder/buildGlowOnStroke.
            --// Motivo: buildGlowOnStroke agrega un UIGradient que ya sube a 0.4 de transparencia en los
            --// extremos; sumado a un pulso propio, el borde quedaba casi invisible en la mayoría de frames.
            --// Este bloque es autocontenido: no toca la función compartida, no afecta a otros íconos.
            do
                ...
                if _capStroke then
                    ...
                    --// Gradiente: sólido en las esquinas curvas, se desvanece en el centro
                    --// de cada línea recta (arriba y abajo). Recalculado para rectángulo
                    --// redondeado 185x42 con radio 14 (4 lados rectos + 4 arcos de 90°):
                    --// línea recta horizontal = 36.52% del perímetro, arco de esquina = 5.12%,
                    --// lado recto vertical = 3.26%. Si el desvanecido queda desplazado,
                    --// ajustar estos keypoints (mapean 0→1 recorriendo el perímetro en sentido horario).
                    local _capGrad = Instance.new("UIGradient")
                    _capGrad.Transparency = NumberSequence.new({
                        NumberSequenceKeypoint.new(0.0000, 0),
                        NumberSequenceKeypoint.new(0.1497, 0.05),
                        NumberSequenceKeypoint.new(0.1826, 0.85),  -- centro linea de arriba
                        NumberSequenceKeypoint.new(0.2155, 0.05),
                        NumberSequenceKeypoint.new(0.3652, 0),     -- fin linea / inicio esquina
                        NumberSequenceKeypoint.new(0.5001, 0),     -- fin lado derecho / inicio linea abajo
                        NumberSequenceKeypoint.new(0.6498, 0.05),
                        NumberSequenceKeypoint.new(0.6827, 0.85),  -- centro linea de abajo
                        NumberSequenceKeypoint.new(0.7155, 0.05),
                        NumberSequenceKeypoint.new(0.8653, 0),
                        NumberSequenceKeypoint.new(1.0000, 0),
                    })
                    _capGrad.Parent = _capStroke
                    ...
                    _capsulePulse:Play()
                    ActiveGlowTweens[_key] = { _capsulePulse }
                end
            end
            return
        end
