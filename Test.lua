...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    --// ── CONTADOR DE ONLINE EN TOPBAR (esquina derecha) ──────────────────
    local TopBarOnlineFrame = mk("Frame", {
    ...
    TopBarOnlineFrame:SetAttribute("ThemeRole", "Secondary")
    corner(TopBarOnlineFrame, 999)

    -- Punto verde de "online"
    local TopBarOnlineDot = mk("Frame", {
    ...
    TopBarOnlineLabel:SetAttribute("ThemeTextRole", "Text")

    -- Animación de pulso en el punto verde
    task.spawn(function()
        while TopBarOnlineDot and TopBarOnlineDot.Parent do
            TweenService:Create(TopBarOnlineDot,
    ...
    -- ═════════════════════════════════════════════════════════════════════
    -- NUEVO BACKEND (v27 - xonokeo.replit.app)
    -- ═════════════════════════════════════════════════════════════════════
    local BACKEND_URL      = "https://yin-yang-backend--xonokeo.replit.app"
    local BACKEND_API_KEY  = "YinYang-GlobalChat-2024-xK9mP3qL"
    local ChatSyncPollRate = 3   -- segundos entre cada consulta de mensajes
    local HeartbeatRate    = 15  -- segundos entre cada heartbeat
    local knownMsgIds      = {}  -- IDs ya renderizados (evita duplicados)
    local backendConnected = false
    local lastTimestamp    = 0   -- solo pedir mensajes más nuevos que este
    ...
    local function backendGet(endpoint)
        local ok, result = pcall(function()
        ...
        local parseOk, data = pcall(HttpService.JSONDecode, HttpService, result.Body)
        return parseOk and data or nil
    end

    local function backendPost(endpoint, body)
        local ok, result = pcall(function()
            return UniversalRequest({
                Url     = BACKEND_URL .. endpoint,
                Method  = "POST",
    ...
    --// Enviar mensaje al nuevo backend
    local function BackendSendMessage(playerName, playerId, message)
        task.spawn(function()
            local avatar = ("rbxthumb://type=AvatarHeadShot&id=%d&w=48&h=48"):format(
                tonumber(playerId) or 0
            )
            local data = backendPost("/messages", {
                username = tostring(playerName),
                avatar   = avatar,
                text     = tostring(message),
                lang     = LanguageSystem.CurrentLanguage or "es",
                game_id  = tostring(game.PlaceId),
            })
            if not data or not data.ok then
                warn("[ChatGlobal] Error al enviar mensaje al backend")
            end
        end)
    end

    --// Heartbeat: avisa al backend que este jugador sigue activo
    local function SendHeartbeat()
        task.spawn(function()
            local data = backendPost("/heartbeat", {
                player_id = tostring(LocalPlayer.UserId),
                username  = LocalPlayer.Name,
                game_id   = tostring(game.PlaceId),
            })
            -- Actualizar contador de online en TopBar y en el chat
            if data and data.online then
                local countStr = tostring(data.online)
                pcall(function() OnlineLabel.Text        = countStr end)
                pcall(function() TopBarOnlineLabel.Text  = countStr end)
            end
        end)
    end

    --// Avisar al backend que el jugador se desconectó
    local function SendDisconnect()
        pcall(function()
            backendPost("/disconnect", {
                player_id = tostring(LocalPlayer.UserId),
            })
        end)
    end

    --// Traducir (sin cambios en la interfaz, solo el endpoint cambia si lo agregas luego)
    local function TranslateMessage(text, targetLanguage, callback)
        -- El nuevo backend no tiene /translate aún; dejamos el callback con error
        -- para que la UI maneje el caso sin romper nada
        task.spawn(function()
            callback(nil, nil, targetLanguage, "translate_not_available")
        end)
    end

    --// Polling principal: pide mensajes nuevos cada ChatSyncPollRate segundos
    local function StartBackendPolling(onNewMessage)
        -- Cargar historial inicial
        task.spawn(function()
            local endpoint = "/messages?limit=100"
            local data = backendGet(endpoint)
            if data and data.messages then
                backendConnected = true
                print("[ChatGlobal] Conectado al backend correctamente")
                for _, msg in ipairs(data.messages) do
                    if not knownMsgIds[msg.id] then
                        knownMsgIds[msg.id] = true
                        if msg.timestamp > lastTimestamp then
                            lastTimestamp = msg.timestamp
                        end
                        -- No renderizar los propios mensajes del historial como "nuevos"
                        -- (se muestran como mensajes normales, no disparan badge)
                        if tostring(msg.username) ~= tostring(LocalPlayer.Name) then
                            pcall(onNewMessage, {
                ...
                warn("[ChatGlobal] No se pudo cargar el historial del backend")
            end
        end)
        ...
        -- Loop de polling + heartbeat con timers
        local pollTimer      = 0
        ...
        RunService.Heartbeat:Connect(function(dt)
            ...
            if heartbeatTimer >= HeartbeatRate then
                heartbeatTimer = 0
                SendHeartbeat()
            end

            if pollTimer >= ChatSyncPollRate then
                pollTimer = 0
                task.spawn(function()
                    local endpoint = "/messages?limit=50"
                    if lastTimestamp > 0 then
                        endpoint = endpoint .. "&since=" .. lastTimestamp
                    end
                    local data = backendGet(endpoint)
                    if data and data.messages then
                        if not backendConnected then
                            backendConnected = true
                            print("[ChatGlobal] Reconectado al backend")
                        end
                        for _, msg in ipairs(data.messages) do
                            if not knownMsgIds[msg.id] then
                                knownMsgIds[msg.id] = true
                                ...
                                -- No re-mostrar los propios mensajes
                                if tostring(msg.username) ~= tostring(LocalPlayer.Name) then
                                    pcall(onNewMessage, {
                                        playerName  = msg.username,
                                        playerId    = 0,
                        ...
                        pcall(function() OnlineLabel.Text       = "?" end)
                        pcall(function() TopBarOnlineLabel.Text = "?" end)
                    end
                end)
            end
        end)
        ...
        game.Players.LocalPlayer.AncestryChanged:Connect(SendDisconnect)
        game:BindToClose(SendDisconnect)
    end
    ...
    StartBackendPolling(function(msg)
        ...
        local name    = msg.playerName or msg.username or "?"
        AddChatMessage(name, msg.playerId or 0, msg.message or msg.text or "", timeStr)
        RenderMessage(name, msg.playerId or 0, msg.message or msg.text or "", timeStr, false)
        -- Badge solo para mensajes nuevos, no del historial inicial
        if not msg.fromHistory then
            addUnread()
        end
    end)
...
local DEMO_ACTIVO = true  -- Demo activada
